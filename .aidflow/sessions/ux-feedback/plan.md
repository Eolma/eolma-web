...
### Phase 1: 토스트 시스템 정상화
- [ ] 1-1: layout.tsx에 ToastProvider 추가 (현재 누락 수정)
- [ ] 1-2: useToastStore 개선 (duration, action, maxCount)
- [ ] 1-3: Toast.tsx 리팩토링 (하단 중앙, framer-motion, 스와이프 dismiss)

### Phase 2: 상태 감지 시스템
- [ ] 2-1: useNetworkStatus 훅 생성
- [ ] 2-2: NetworkBanner 컴포넌트 생성, layout.tsx에 적용
- [ ] 2-3: useAuctionSocket connectionStatus enum 리팩토링
- [ ] 2-4: ConnectionIndicator 컴포넌트 생성, BidPanel에 적용

### Phase 3: Optimistic UI
- [ ] 3-1: useAuctionSocket placeBid에 낙관적 업데이트 + 롤백 로직
- [ ] 3-2: BidPanel에 isPending 상태 추가

### Phase 4: 폼 진행률
- [ ] 4-1: FormProgress 컴포넌트 생성
- [ ] 4-2: ProductForm에 FormProgress 적용

### Phase 5: 검증
- [ ] 5-1: npm run build 확인
- [ ] 5-2: 스모크 테스트 (토스트 표시, 네트워크 끊김, 입찰 Optimistic, 폼 진행률)

## Acceptance Criteria
- [ ] AC-1: 입찰 버튼 클릭 즉시 가격이 변경되고, 실패 시 원래 값으로 돌아감
- [ ] AC-2: 토스트가 하단 중앙에 표시되고 좌우 스와이프로 닫을 수 있음
- [ ] AC-3: 토스트에 액션 버튼이 표시될 수 있음 (예: "취소" 버튼)
- [ ] AC-4: 상품 등록 폼 상단에 진행률 바가 표시되고 필드 입력에 따라 증가함
- [ ] AC-5: 인터넷 연결이 끊기면 상단에 빨간 배너가 표시됨
- [ ] AC-6: WebSocket 연결 상태가 컬러 dot으로 시각화됨
